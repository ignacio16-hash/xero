...
// Falabella status mapping — update when docs are provided
...
function normalize(payload: Record<string, unknown>): NormalizedOrder {
  // TODO: replace field paths once Falabella API docs are provided
...
export async function handleFalabellaWebhook(req: Request, res: Response) {
  try {
    const order = await upsertOrder(normalize(req.body));
    res.status(200).json({ ok: true, orderId: order.id });
  } catch (err) {
    console.error('[falabella] webhook error:', err);
